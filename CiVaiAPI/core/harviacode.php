...
class Harviacode
{
    ...
    function insert_menu()
    {
        if (isset($_POST['controller'])) {
            ...
            $jenis_api = $_POST['jenis_api'];
            //$icon = 'fal fa-align-justify';
            $create_date = date('Y-m-d H:i:s');
            $aktif = 'y';
            $query = "INSERT INTO tbl_api_menu values(?,?,?,?,?,?)";
            $stmt = $this->sql->prepare($query);
            $stmt->bind_param('ssssss', $id, $title, $url, $jenis_api, $create_date, $aktif);
            $stmt->execute();
            $stmt->close();
            $this->sql->close();
        }
    ...
    function menumain()
    {
        ...
        $url = $_POST['controller'];
        $sql = "SELECT id FROM tbl_api_menu where url='" . $url . "'";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            $nm_tabel = $_POST['table_name'];
            $title = $_POST['tombol'];
            $url = $_POST['controller'];
            $jenis_api = $_POST['jenis_api'];
            $create_date = date('Y-m-d H:i:s');
            $sqlx = "UPDATE tbl_api_menu set nm_tabel='" . $nm_tabel . "',title='" . $title . "',tipe='" . $jenis_api . "',create_date='" . $create_date . "' where url='" . $url . "'";
            ...
            $create_date = date('Y-m-d H:i:s');
            $aktif = "y";
            $sqlx = "INSERT INTO tbl_api_menu (`nm_tabel`,`title`,`url`,`tipe`,`create_date`,`is_aktif`) values('" . $nm_tabel . "','" . $title . "', '" . $url . "', '" . $jenis_api . "', '" . $create_date . "', '" . $aktif . "')";
            $result = $conn->query($sqlx);
        }
        $conn->close();
    }
